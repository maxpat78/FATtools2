...
class boot_fat16(object):
	...
	def __init__ (self, diskstream, offset=0):
		self._i = 0
		self._pos = diskstream.tell() # base offset
		self._buf = diskstream.read(512) # normal boot sector size
...
class FAT(object):
	...
	def __init__ (self, stream, offset, clusters, bitsize=32, exfat=0):
		# Separate cached disk stream
		self.stream = disk.disk(stream._file.name, stream._file.mode)
	...
	def findfree(self, start=2, count=0): # in exFAT we must look at the free clusters bitmap!
		...
		while 1:
			...
			if self[start] == 0:
				...
				break
			start += 1
		#~ logging.debug("found %d free clusters from #%x", n, i)
		return i, n
	...
	def findmaxrun(self):
		...
		while 1:
			...
			if t[0] < 0: break
			logging.debug("Found %d free clusters from #%d", t[1], t[0])
	...
	def alloc(self, count, start=2, first=0):
		...
		#~ logging.debug("request to allocate %d clusters from #%Xh(%d), search from #%Xh", count, start, start, self.last_free_alloc)
		...
		if count:
			...
			logging.debug("FATAL: couldn't allocate %d more clusters", count)
			self.free(first)
			return 0
		#~ logging.debug("clusters successfully allocated from %Xh", first)
		#~ print first, last
		return first

	def free(self, start):
		"Free a clusters chain"
		#~ logging.debug("freeing cluster chain from %Xh", start)
		while not (self.last <= self[start] <= self.last+7): # islast
			prev = start
			start = self[start]
			self[prev] = 0
			#~ logging.debug("freed cluster %x", prev)
		self[start] = 0
...
class Chain(object):
	...
	def read(self, size=-1):
		...
		if self.nofat: # contiguous clusters
			...
			self.pos += size
			#~ logging.debug("read %d contiguous bytes, VCN=%Xh[%Xh:]", len(buf), self.vcn, self.vco)
			return buf
		while 1:
			if not size or self.vcn == -1:
				break
			...
			self.pos += n
			self.seek(self.pos)
		#~ logging.debug("read %d byte, VCN=%Xh[%Xh:]", len(buf), self.vcn, self.vco)
		return buf
...
class FixedRoot(object):
	...
	def seek(self, offset, whence=0):
		if whence == 1:
			...
			pos = offset
		if pos > self.size:
			logging.debug("Attempt to seek @%Xh past fixed root end @%Xh", pos, self.size)
			return
		self.pos = pos
...
class FATDirentry(Direntry):
	...
	def ShortName(self):
		return self.GenShortName(self._buf[-32:-21])
	...
	@staticmethod
	def GenShortName(shortname):
		"Make a human readable short name from slot's one"
		shortname=str(shortname)
		if len(shortname) > 8 and shortname[-3] != ' ':
			# NNNNNNNNEEE --> NNNNNNNN.EEE
			return shortname[:8].rstrip() + '.' + shortname[8:].rstrip()
		else:
			return shortname.rstrip()
...
class Dirtable(object):
	...
	def findfree(self):
		...
		while s:
			...
			if not s or s[0] == 0:
				#~ logging.debug("Found next Dirtable's free slot @%Xh", found)
...
def opendisk(path, mode='rb'):
    ...
    fstyp = utils.FSguess(boot_fat16(d)) # warning: if we call this a second time on the same Win32 disk, handle is unique and seek set already!
    d.seek(0)
